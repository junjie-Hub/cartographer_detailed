...
int MapBuilderBridge::AddTrajectory(
  ...
  // 检查trajectory_id 确保之前没有使用过
  CHECK_EQ(sensor_bridges_.count(trajectory_id), 0);

  // SensorBridge将ROS的消息转换成Cartographer中的传感器数据类型
// 最后的参数是Cartographer的核心对象，TrajectoryBuilderInterface指针
// 通过sensor_bridge对象转换后的数据都是通过它喂给Cartographer的
  sensor_bridges_[trajectory_id] = absl::make_unique<SensorBridge>(
      ...
      // 轨迹的配置保存到容器对象 trajectory_options_
  auto emplace_result =
...
void MapBuilderBridge::OnLocalSlamResult(
    const int trajectory_id, const ::cartographer::common::Time time,// 更新子图的时间
    const Rigid3d local_pose, // 子图的参考位置
    ::cartographer::sensor::RangeData range_data_in_local) {   // 参考位置下的扫描数据

    // LocalSlamData是定义在MapBuilderBridge内部的结构体，用于记录局部SLAM反馈的状态                                  
  std::shared_ptr<const LocalTrajectoryData::LocalSlamData> local_slam_data =
